...
function updatePrompt() {
  ...
  local GIT_COMMIT_HASH=${git_status_fields[10]}
  ...
  if [[ "${#git_status_fields[@]}" -gt 0 ]]; then
    ...
    case "${GIT_BRANCH-}" in
      "${GIT_PROMPT_MASTER_BRANCHES}")
        local STATUS_PREFIX="${PROMPT_LEADING_SPACE}${GIT_PROMPT_PREFIX_FINAL}${GIT_PROMPT_MASTER_BRANCH}${URL_SHORT-}\${GIT_BRANCH}${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_COMMIT_HASH}${GIT_COMMIT_HASH}${ResetColor}${GIT_FORMATTED_UPSTREAM-}"
        ;;
      *)
        local STATUS_PREFIX="${PROMPT_LEADING_SPACE}${GIT_PROMPT_PREFIX_FINAL}${GIT_PROMPT_BRANCH}${URL_SHORT-}\${GIT_BRANCH}${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_COMMIT_HASH}${GIT_COMMIT_HASH}${ResetColor}${GIT_FORMATTED_UPSTREAM-}"
        ;;
    esac
    local STATUS=""
